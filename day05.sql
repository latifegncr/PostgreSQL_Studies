...
malzeme_isim= (select sirket_ismi from toptancilar where 
			   irtibat_ismi ='Adem Coş'  ) where musteri_isim = 'Ali Can';
...
update malzemeler set
musteri_isim= (select irtibat_ismi from toptancilar where 
			   sirket_ismi ='Apple'  ) where malzeme_isim = 'Laptop';
...
select * from meslekler ORDER BY isim ASC;
...
--SORU5(ÇÖZMEEE): meslekler tablosunu, 
--isim kelime uzunluguna gore kucukten buyuge sıralayarak listeleyiniz
select * from meslekler ORDER BY length(isim) ASC;
...
--Limit : sınır koymak için yazılır
...
insert into  values(10,'Adem',70000);
...
--Satır atlamak istediğimizde 'offset' komutunuz kullanırız
...
--INTERSECT : İki sorgu ifadesinin  kesişim değerini verir.

--Except : İlk sorguda olup ikinci sorguda olmayan değeri verir.
...
-- SORU1: isim'i 'Murat Oz' olanlarin isim'lerini, sehir'lerini ve
 ...
 select isim, sehir from isciler where
 isim = 'Murat Oz' intersect select isim, sehir from isciler where
 sehir <>  'Istanbul';
 ...
 select isim, sehir from isciler where
 isim = 'Murat Oz' EXCEPT select isim, sehir from isciler where
 sehir <>  'Istanbul';
