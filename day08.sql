...
--SORU4: soyisim sutununu ilk harfi buyuk diger harfleri küçük listeleyiniz.
...
IS NULL : Null olan verileri getirir bize.

IS NOT NULL : Null olmayan verileri getirir bize.
...
select film_name, actor_name from aktorler inner join filmler on aktorler.film_id  =filmler.film_id;

--SORU4: Tüm film_name'leri ve tüm actor_name'leri listeleyiniz
select film_name, actor_name from aktorler full join filmler  on aktorler.film_id  =filmler.film_id;
...
Begin ;   --Transaction 'ı başlatmak için kullandık.
...
savepoint x; -- Çok önemli verileri silinmeye karşı save eder.
...
rollback to x; --Silinen çok önemli verileri geri getirir.
