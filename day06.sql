...
--Her satır bir bireyi temsil etiği için id, isim vs. istediğimiz sütuna göreyapabiliri.


-- SORU03: cinsiyet'e göre maas ortalamasını listeleyiniz
select cinsiyet, round(avg(maas)) as ortalamaMaas from insan group by cinsiyet;
...
select ulke,sehir, count(isim) as calisansayisi from insan group by ulke,sehir;

-- SORU06: ulke'lere göre ve 
--cinsiyet'e göre çalışan sayısını ve yas ortalamasını listeleyiniz.
select ulke, cinsiyet, count(isim) as calisanSayisi, avg(yas) as yasOrtalaması from insan group by ulke, cinsiyet;

-- SORU07: maas'ı 30000 den büyük olanların, 
-- ulke'lere göre ve cinsiyet'e göre çalışan sayısını, yas ortalamasını listeleyiniz.
select ulke, cinsiyet, count(isim) as calisanSayisi,
round(avg(yas)) as yasOrtalaması from insan where
maas>30000 group by ulke, cinsiyet;
...
--SORU08: ulke'lere göre yas ortalamasını bulup,
---yas ortalaması 33'den buyuk olanları listeleyiniz. 
select ulke, avg(yas) from insan where  group by ulke having  round(avg(yas))>30 ;

--SORU9: ulke'lere göre ortalama maası bulup, ortalama maas 30000'den buyuk olanları listeleyiniz
select ulke, round(avg(maas)) as ortalama_maas from
insan  group by ulke  having avg(maas)>30  ;
-- Büyükten küçüğe sıralamak istersekt de kodun en sona "order by ortalama_maas desc" koyabiliriz.
